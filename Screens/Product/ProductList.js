...
const ProductList = ({ searchQuery, priceRange, selectedCategories }) => {
    const [products, setProducts] = useState([]);
    const [filteredProducts, setFilteredProducts] = useState([]);
    const [loading, setLoading] = useState(true);
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                const res = await axios.get(`${baseURL}/product/get/all`);
                setProducts(res.data.products);
                setFilteredProducts(res.data.products); // Display all products by default
            } catch (error) {
                console.error('Failed to fetch products:', error);
            } finally {
                setLoading(false);
    ...
    }, [searchQuery, priceRange, selectedCategories]); // Only filter when these values change
